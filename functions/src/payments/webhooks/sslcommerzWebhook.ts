...
export const sslcommerzWebhook = onRequest(
  ...
  async (req, res) => {
    ...
    // error so SSLCommerz stops retrying; log sanitised error for ops).
    try {
      const rawBody =
      ...
      const validated = valId
        ? await validate(valId).catch((err) => {
            console.error("[sslcommerzWebhook] validate failed:", sanitise(err));
            return null;
          })
        : null;
      ...
      if (!validated || validated.status !== "VALID") {
        await db
          ...
          .set({
            ...
            validatedPayload: validated ? sanitise(validated) : null,
            receivedAt: Date.now(),
          });
      ...
      // SSLCommerz IPN payloads contain `store_id` + `verify_key` (effectively
      // credentials); the validate() response can also echo them back. Sanitise
      // before logging.
      console.error("[sslcommerzWebhook] unhandled error:", sanitise(err));
      if (!res.headersSent) {
