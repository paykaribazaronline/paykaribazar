...
export const nagadWebhook = onRequest(
  ...
  async (req, res) => {
    // Top-level try/catch — see bkashWebhook.ts for rationale (always 200 on
    // error so Nagad stops retrying; log sanitised error for ops).
    try {
      const rawBody =
      ...
      const verified = await verify(paymentRefId, merchantOrderId, amountPoisha).catch(
        (err) => {
          console.error("[nagadWebhook] verify failed:", sanitise(err));
          return null;
        },
      );
      ...
      if (!verified || String(verified.status ?? "").toLowerCase() !== "success") {
        await db
          ...
          .set({
            ...
            verifiedPayload: verified ? sanitise(verified) : null,
            receivedAt: Date.now(),
          });
      ...
      // Nagad's encrypted `sensitiveData` blob would already be decrypted by
      // this point if verify() succeeded — sanitise to redact `signature` /
      // `sensitiveData` / `merchantId` / any token fields before logging.
      console.error("[nagadWebhook] unhandled error:", sanitise(err));
      if (!res.headersSent) {
